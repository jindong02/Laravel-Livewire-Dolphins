...
<div class="d-flex" style="gap:4px">
    <a href="{{ route('approvals.department.index') }}" class="btn {{ !request()->get('view') ? 'btn-outline-secondary' : 'btn-secondary'}}" style="width: 200px">LOT</a>
    <a href="{{ route('approvals.department.index') . '?view=LINE' }}" class="btn {{ request()->get('view') !== 'LINE' ? 'btn-secondary' : 'btn-outline-secondary'}}" style="width: 200px">LINE</a>
</div>
<br/>

<form action="{{ route('approvals.department.validate')}}" method="POST" id="approval-validation-form">
    @csrf
    ...
    @if (request()->get('view') !== 'LINE')
        <x-approval-basic-lot-list-component :collection="$itemRequests" :approver="'department'"/>
    @else
        <x-approval-basic-line-list-component :collection="$itemRequests" :approver="'department'"/>
    @endif
